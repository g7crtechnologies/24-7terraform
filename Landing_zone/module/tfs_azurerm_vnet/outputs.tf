...
output "app-gw" {
  value      = "hub" == local.env_name[terraform.workspace] ? local.subnet_id.2 : null
  depends_on = [local.hub_con]
}
output "lb" {
  value      = "hub" == local.env_name[terraform.workspace] ? local.subnet_id.3 : null
  depends_on = [local.hub_con]
}
output "gateway" {
  value      = "hub" == local.env_name[terraform.workspace] ? local.subnet_id.4 : null
  depends_on = [local.hub_con]
}
